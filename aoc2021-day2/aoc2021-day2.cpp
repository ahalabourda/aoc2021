...
void puzzle1() {
    ...
    enum Direction { Invalid = -1, Forward, Up, Down };
...
void puzzle2() {
    ...
    for (unsigned int i = 0; i < allCommands.size(); i++) {
        ...
        case Forward:
            distance += allCommands[i].distance;
            depth += currentAim * allCommands[i].distance;
            break;
        case Down:
            currentAim += allCommands[i].distance;
            break;
        case Up:
            currentAim -= allCommands[i].distance;
            break;
        ...
        cout << "adding " << allCommands[i].distance << " to " << allCommands[i].direction << " our current aim is " << currentAim << "\r\n";
